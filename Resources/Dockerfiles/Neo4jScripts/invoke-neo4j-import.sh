...
until cat /scripts/neo4j-db-config | /var/lib/neo4j/bin/cypher-shell --format plain  -u neo4j -p $NEO4J_PWD >> dbconfig.log
do
    echo "db config failed, sleeping"
    sleep 10
done

until cat /scripts/neo4j-import | /var/lib/neo4j/bin/cypher-shell --format plain -u neo4j -p $NEO4J_PWD >> import.log
do
